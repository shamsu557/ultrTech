...
function updatePaymentOptions(result) {
  ...
  if (!student.admission_number) {
    ...
    if (proceedPaymentBtn) proceedPaymentBtn.style.display = paymentType ? "block" : "none";
    return;
  }
  ...
  if (student.installmentNumber === 1 && totalPaid < regFee) {
    ...
    if (letterBtn) letterBtn.style.display = "inline-block"; // Show admission letter after first installment
    if (messageBox) {
      messageBox.innerHTML = `<p class="text-info">First installment paid. You may download your admission letter and receipt below. Complete the second installment to proceed.</p>`;
    }
    return;
  }

  if (totalPaid >= regFee) {
    if (student.installmentNumber === 2) {
      if (messageBox) {
        messageBox.innerHTML = `<p class="text-success fw-bold">✅ Second installment payment successful! Your registration payment is complete.</p>`;
      }
      return; // No buttons shown for second installment
    }
    if (messageBox) {
      messageBox.innerHTML = `<p class="text-success fw-bold">✅ You have completed your registration payment. You may download your receipt and admission letter below.</p>`;
    }
    if (receiptBtn) receiptBtn.style.display = "inline-block";
  ...
  if (messageBox) {
    messageBox.innerHTML = `<p class="text-warning">Please contact support or try again.</p>`;
  }
}
  ...
  try {
    ...
    if (result.success) {
      ...
      if (admissionNum) {
        ...
        sessionStorage.setItem("admissionNumber", admissionNum);
      }
      ...
      if (successModalEl) {
        const admissionSpan = successModalEl.querySelector('#admissionNumberSpan');
        if (admissionSpan) {
          admissionSpan.innerHTML = `<strong>Admission Number:</strong> ${admissionNum || 'N/A'}`;
        } else {
          console.warn("Admission number span not found in modal");
        }
...
function showSecuritySetup(studentId, admissionNumber) {
  ...
  container.innerHTML = `
    ...
    <form id="securityForm">
      ...
      <input type="file" id="qualificationFile" class="form-control" accept=".pdf,.jpg,.png" required />

      <button type="submit" class="btn btn-primary mt-3">Complete Registration</button>
    </form>
  `;
  ...
  document.getElementById("securityForm").addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const securityRes = await fetch(`${BASE_URL}/api/student/setup-security`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ studentId, password, securityQuestion: question, securityAnswer: answer }),
      });

      if (!securityRes.ok) {
        const result = await securityRes.json().catch(() => ({ error: `HTTP ${securityRes.status}` }));
        throw new Error(result.error || `HTTP ${securityRes.status}`);
      }

      // Complete registration with upload
      const formData = new FormData();
      formData.append("studentId", studentId);
      formData.append("highestQualification", file);

      const completeRes = await fetch(`${BASE_URL}/api/student/complete-registration`, {
        method: "POST",
        body: formData,
      });

      if (!completeRes.ok) {
        const data = await completeRes.json().catch(() => ({ error: `HTTP ${completeRes.status}` }));
      ...
      if (data.success) {
        ...
        studentData.admission_number = admissionNum;
        showMessage(`Registration completed! Your Admission Number is: ${studentData.admission_number}`, "success");

        container.innerHTML = `
          <h3>Registration Completed ✅</h3>
          <p><strong>Admission Number:</strong> ${admissionNum}</p>
          <button id="downloadAdmissionLetterBtn" class="btn btn-success mt-2">Download Admission Letter</button>
        `;

        // attach listener properly
        document.getElementById("downloadAdmissionLetterBtn")
          .addEventListener("click", downloadAdmissionLetter);
      } else {
...
function downloadAdmissionLetter() {
  const adm = studentData?.admission_number || sessionStorage.getItem("admissionNumber");
  ...
  const url = `${BASE_URL}/api/admission-letter/download?admissionNum=${encodeURIComponent(adm)}`;
  fetch(url, { method: "HEAD" })
    ...
    .catch(err => {
      ...
      if (alertBox) {
        alertBox.innerHTML = `
          <div class="alert alert-danger">Failed to download admission letter: ${err.message}</div>
        `;
      }
    });
}
...
function downloadRegistrationReceipt() {
  ...
  if (!adm) {
    ...
    if (alertBox) {
      alertBox.innerHTML = `
        <div class="alert alert-danger">Registration payment incomplete or admission number missing. Cannot download receipt.</div>
      `;
    }
    return;
  }
  ...
  fetch(url, { method: "HEAD" })
    ...
    .catch(err => {
      ...
      if (alertBox) {
        alertBox.innerHTML = `
          <div class="alert alert-danger">Failed to download receipt: ${err.message}</div>
        `;
      }
    });
}
...
/* -------------------------
   ...
   ------------------------- */
document.addEventListener("DOMContentLoaded", () => {
  const admBtn = document.getElementById("downloadAdmissionLetterBtn");
  if (admBtn) admBtn.addEventListener("click", downloadAdmissionLetter);
