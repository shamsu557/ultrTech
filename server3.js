...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (mimetype && extname) {
      ...
      cb(new Error("Invalid file type. Only JPG, JPEG, PNG, JFIF, PDF, DOC, DOCX, or ZIP are allowed."));
    }
  },
});
...
function generateAdmissionNumber(abbreviation, certType) {
  ...
  return `${abbreviation}/${certType}/${year}/${randomNum}`;
}
...
// Verify application or admission for registration fee
app.get("/api/student/verify-application/:identifier", (req, res) => {
  console.log(`Received request for /api/student/verify-application/${req.params.identifier} with query:`, req.query);
  ...
  if (type === "application") {
    query = `
      SELECT s.*, c.name as course_name, c.registration_fee, c.duration, c.certification_type 
      FROM students s 
  ...
  db.query(query, params, (err, results) => {
    ...
    if (results.length === 0) {
      return res.status(404).json({ success: false, error: "Application or admission number not found" });
    }

    const student = results[0];

    const paymentQuery = `
      SELECT SUM(amount) as total_paid, MAX(installment_number) as max_installment, MAX(total_installments) as total_installments 
      FROM payments 
    ...
    db.query(paymentQuery, [student.id], (err, paymentResults) => {
      ...
      const maxInstallment = paymentResults[0].max_installment || 0;
      const totalInstallments = paymentResults[0].total_installments || 0;
      const registrationFee = student.registration_fee;

      let paymentStatus = 'Pending';
      let paymentOptions = ['Full', 'Installment'];
      let message = '';

      if (totalPaid >= registrationFee) {
        paymentStatus = 'Completed';
        paymentOptions = [];
        message = 'You have made the full registration payment';
      } else if (totalPaid > 0) {
        paymentStatus = 'Partial';
        if (maxInstallment === 1 && totalInstallments === 2) {
          paymentOptions = ['Installment'];
          message = 'First installment is made, proceed to second installment';
        } else {
          paymentOptions = ['Full', 'Installment'];
          message = 'Partial payment made, complete your registration payment';
        }
      } else {
        paymentOptions = ['Full', 'Installment'];
        message = 'No registration payment made yet';
      }
...
// Verify admission for registration fee
app.get("/api/student/verify-admission/:admissionNumber", (req, res) => {
  ...
  db.query(query, [admissionNumber], (err, results) => {
    if (err) {
      console.error("Error verifying admission:", {
        error: err.message,
        code: err.code,
        sqlMessage: err.sqlMessage,
        sqlState: err.sqlState,
    ...
    if (results.length === 0) {
      return res.status(404).json({ success: false, error: "Admission number not found" });
    }

    const student = results[0];

    if (!student.application_number) {
      return res.status(400).json({ success: false, error: "No application number associated with this admission number" });
    }
...
app.post("/api/payment/verify", async (req, res) => {
  try {
    ...
    if (paymentType === "Application") {
      ...
      db.query(courseCheckQuery, [applicationData.courseId], (err, courseResults) => {
        ...
        if (courseResults.length === 0) {
          return res.status(400).json({
            success: false,
            error: `Invalid course ID: ${applicationData.courseId}`,
          });
        }
      ...
      // Verify student exists and has an application number
      const studentCheckQuery = `SELECT course_id FROM students WHERE id = ? AND application_number IS NOT NULL`;
