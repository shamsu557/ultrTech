...
async function verifyApplication(e) {
  ...
  const verifyTypeSelected = document.getElementById("verifyType")?.value; // "application" or "admission" from UI select
  ...
  // Detect admission format: e.g., CA/2025/CERT/967 or similar
  const admissionNumberRegex = /^[A-Z0-9]{2,5}\/\d{4}\/[A-Z]{3,6}\/\d{1,4}$/i;
  const applicationNumberRegex = /^[A-Z0-9\-]+$/i;

  // Decide type to query backend with -- prefer the explicit selection but auto-correct if it looks like an admission number
  ...
  try {
    ...
    if (!result.success) {
      // If backend says not found for the type we used, and user selected application, try the other type once
      if (typeToUse === "application") {
        // try admission fallback
...
function processVerificationResult(result) {
  // Normalize and store student data
  ...
  // Unified messaging — stay on Step 2 (payment) by default; only guide the user
  if (s.admission_number) {
    if (s.totalPaid >= registrationFee) {
      ...
      showStep(2); // stay on payment step, let user choose to go to step 3 manually
...
function updatePaymentOptions(result) {
  ...
  // CASE: no admission number -> no completed registration yet
  if (!student.admission_number) {
    // Show both options (Full, Installment first)
  ...
  // If admission number exists, at least one payment was recorded.
  // If totalPaid >= regFee => fully paid (either full or after second installment)
  if (totalPaid >= regFee) {
  ...
  // Partial payment case (first installment paid)
  // Show only second installment option + show receipt for the payment already made
  if (student.installmentNumber === 1 && totalPaid < regFee) {
    ...
    if (proceedPaymentBtn) proceedPaymentBtn.style.display = "block";
    if (receiptBtn) receiptBtn.style.display = "inline-block"; // allow downloading receipt for partial payment
    return;
  }

  // Fallback: show no payment options, show warning
...
/* -------------------------
   ...
   (important fix: do not auto-jump to step 3/4)
   ------------------------- */
async function verifyRegistrationPayment(reference, installmentType) {
  ...
  try {
    ...
    // normalize student and merge into studentData
    ...
    // Behavior:
    // - first installment -> stay on step 2 and ask for second
    // - second OR full -> show completed message + receipt/letter, stay on step 2 (do NOT auto-jump)
    if (installmentType === "first") {
      showMessage("✅ First installment payment verified. Please pay the second installment.", "success");
      showStep(2);
      ...
      updatePaymentOptions({ student: studentData });
      showStep(2); // stay here — user can go to steps 3 & 4 manually if needed
    }
  } catch (err) {
...
/* -------------------------
   Application fee flow (application modal)
   ------------------------- */
  ...
  payButton.onclick = async () => {
    try {
      ...
      const handler = PaystackPop.setup({
        key: "pk_live_661e479efe8cccc078d6e6c078a5b6e0dc963079",
        email,
        amount: 10000, // application fee in naira * 100? adjust if needed
        currency: "NGN",
...
/* -------------------------
   Security setup (always required once)
   ------------------------- */
...
/* -------------------------
   Complete registration (upload qualifications)
   ------------------------- */
async function completeRegistration(e) {
  ...
  // Optional: ensure payment is at least first installment or fully paid depending on your business rule.
  // Here we allow completion if student has admission_number (indication of first payment) or fully paid.
  ...
  try {
    ...
    const result = await res.json();
    if (result.success) {
      // server returns admissionNumber on success
      ...
      // update UI (show receipt/admission)
      updatePaymentOptions({ student: studentData });
